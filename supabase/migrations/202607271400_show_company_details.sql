...
alter table public.cards
  add column if not exists show_company_details boolean not null default true;
...
begin
  ...
  insert into public.cards (
    ...
    coalesce(trim(p_bio), ''),
    p_theme_color,
    coalesce(p_profile_image_url, ''),
    coalesce(p_company_logo_url, ''),
    coalesce(p_cover_image_url, ''),
    coalesce(p_show_company_details, true),
    'published',
    now()
  )
...
revoke all on function public.publish_my_card(text, text, text, text, text, text, text, text, text, jsonb, boolean) from public, anon;
grant execute on function public.publish_my_card(text, text, text, text, text, text, text, text, text, jsonb, boolean) to authenticated;
