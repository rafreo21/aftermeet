...
function explainNfcFailure(error: unknown) {
  ...
  if (/ndef|format/i.test(message)) {
    return Platform.OS === 'ios'
      ...
      : 'This tag could not be prepared for an AfterMeet link. Try a blank NTAG213, NTAG215, or NTAG216 tag.';
  }
  return message || 'AfterMeet could not program this tag. Hold one writable NFC tag near the phone and try again.';
}
...
export async function programNfcTag(
  ...
  if (!isNativeNfcSupported()) {
    throw new Error('NFC programming is only available in the AfterMeet iOS and Android apps.');
  }
