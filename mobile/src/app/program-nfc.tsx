...
function stateDescription(state: ScreenState) {
  switch (state) {
    case 'checking-device': return 'AfterMeet is checking NFC support and whether it is enabled.';
    ...
    case 'reading-tag': return 'Keep the tag still while AfterMeet checks that it can be written.';
    case 'writing-tag': return 'Do not move the tag. AfterMeet is saving your live card link.';
    case 'verifying-tag': return 'Keep the tag in place while AfterMeet confirms the card link.';
    ...
    default: return 'Use one blank or rewritable NDEF tag. AfterMeet will save your live card link and verify it before finishing.';
  }
}
...
export default function ProgramNfcScreen() {
  ...
  async function startProgramming() {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : 'AfterMeet could not program this NFC tag.');
      setState('error');
