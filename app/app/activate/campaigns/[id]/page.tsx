...
export default function CampaignDetailPage() {
  ...
  if (!campaign || !attribution) {
    return (
      <AppShell active="activate" title="Campaign" actions={<LinkButton size="small" variant="ghost" href="/app/activate/campaigns"><ArrowLeftIcon size={16} />Campaigns</LinkButton>}>
        <div className="empty-state"><div><h2>Campaign not found</h2><LinkButton href="/app/activate/campaigns">Back to campaigns</LinkButton></div></div>
      </AppShell>
    );
  }
  ...
  function archive() {
    ...
    window.location.href = "/app/activate/campaigns";
  }
          ...
          {campaignContacts.length ? (
            <div className="campaign-people-list">
              {campaignContacts.slice(0, 8).map((contact) => (
                <LinkButton key={contact.id} variant="ghost" href={`/app/contacts/${contact.id}`}>
